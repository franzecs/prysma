...
export class EmpresasListComponent implements OnInit, pageConfig {
  ...
  getList() {
    ...
    this.subscription = this.empresasService.getList(PATH_EMPRESA)
                          .subscribe(resultado => {
  ...
  openModal(idOBJ, nome){
    this.idObj = idOBJ
    this.modalMsg.showMessage("Deseja realmente exluir a Empresa: " + nome, MsgType.DELETE)
  }  

  filter(evento){
    this.objFilter = this.empresas.filter(obj => new RegExp(evento).test(obj['nome']))
    this.fillTable()
  } 
}